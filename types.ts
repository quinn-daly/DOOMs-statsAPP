...
export interface GameEvent {
  ...
  throwPurpose?: ThrowPurpose | null;
  turnCause?: TurnCause | null;
  pressureCreditPlayerId?: number | null;
  blockType?: BlockType | null;
  scoredOnDefenderId?: number | null;
}
...
export function actionNeedsThrow(action: Action): boolean {
  return action === 'Catch' || action === 'Throwaway' || action === 'Goal' || action === 'Drop';
}

export function actionNeedsTurnCause(action: Action): boolean {
  return action === 'Throwaway' || action === 'Drop';
}

export function actionNeedsBlockType(action: Action): boolean {
  return action === 'D' || action === 'Callahan';
}

export function actionNeedsReceiver(action: Action): boolean {
  return action === 'Catch' || action === 'Goal' || action === 'Drop';
}

export function actionNeedsDefender(action: Action): boolean {
  return action === 'D' || action === 'Callahan';
}
