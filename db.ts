...
export async function getAllRosters(): Promise<Roster[]> {
  return db.rosters.orderBy('updatedAt').reverse().toArray();
}

export async function createRoster(name: string): Promise<number> {
  const now = Date.now();
  return db.rosters.add({ name, createdAt: now, updatedAt: now });
}

export async function updateRoster(id: number, name: string): Promise<void> {
...
export async function deleteRoster(id: number): Promise<void> {
  await db.transaction('rw', db.rosters, db.players, db.games, db.events, async () => {
    ...
    for (const g of games) {
      await db.events.where('gameId').equals(g.id!).delete();
    }
...
export async function getPlayersForRoster(rosterId: number): Promise<Player[]> {
  return db.players.where('rosterId').equals(rosterId).toArray();
}

export async function createPlayer(data: Omit<Player, 'id' | 'createdAt' | 'updatedAt'>): Promise<number> {
  const now = Date.now();
  return db.players.add({ ...data, createdAt: now, updatedAt: now });
}

export async function updatePlayer(id: number, data: Partial<Player>): Promise<void> {
  await db.players.update(id, { ...data, updatedAt: Date.now() });
}
...
export async function createGame(data: Omit<Game, 'id' | 'createdAt' | 'updatedAt'>): Promise<number> {
  const now = Date.now();
  return db.games.add({ ...data, createdAt: now, updatedAt: now });
}

export async function updateGame(id: number, data: Partial<Game>): Promise<void> {
  await db.games.update(id, { ...data, updatedAt: Date.now() });
}
...
export async function duplicateGame(id: number): Promise<number> {
  ...
  for (const evt of events) {
    const { id: _eid, ...rest } = evt;
    await db.events.add({ ...rest, gameId: newGameId });
  }
  return newGameId;
}
...
export async function getEventsForGame(gameId: number): Promise<GameEvent[]> {
  return db.events.where('gameId').equals(gameId).sortBy('seq');
}

export async function getNextSeq(gameId: number): Promise<number> {
  const events = await db.events.where('gameId').equals(gameId).toArray();
  if (events.length === 0) return 1;
  return Math.max(...events.map(e => e.seq)) + 1;
}

export async function addEvent(data: Omit<GameEvent, 'id'>): Promise<number> {
  const id = await db.events.add(data as GameEvent);
  await db.games.update(data.gameId, { updatedAt: Date.now() });
  return id;
}

export async function updateEvent(id: number, data: Partial<GameEvent>): Promise<void> {
  const evt = await db.events.get(id);
  if (evt) {
    await db.events.update(id, data);
    await db.games.update(evt.gameId, { updatedAt: Date.now() });
  }
}

export async function deleteLastEvent(gameId: number): Promise<void> {
